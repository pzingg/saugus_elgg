...
$ownedusers = run("ownedusers:get",array($page_owner));

// remmed - don't want to see my posts'
//$where2 = "weblog = '$page_owner'";
$where2 = "";
        
if (!empty($ownedusers)) {
    foreach($ownedusers as $owneduser) {
        
        if ($where2 != "") {
            $where2 .= "or ";
        }
        $where2 .= "owner='$owneduser->ident'";
    }
    
}

$where1 = run("users:access_level_sql_where",$USER->ident);
...
if (!empty($posts)) {
    ...
    if ($numberofposts - ($weblog_offset + 25) > 0) {
        ...
        $run_result .= <<< END
                ...
                <a href="{$CFG->wwwroot}{$weblog_name}/weblog/ownedusers/skip={$display_weblog_offset}">&lt;&lt; $back</a>
...
END;
    ...
    if ($weblog_offset > 0) {
        ...
        $run_result .= <<< END
                ...
                <a href="{$CFG->wwwroot}{$weblog_name}/weblog/ownedusers/skip={$display_weblog_offset}">$next &gt;&gt;</a>
