...
END;
    ...
    $emailReplies = gettext("Receive email notifications");
        $emailRules = gettext("Set this to 'yes' if you would like to receive notifications whenever anyone adds you as a friend or applies to join a community you own through your email.");
...
END;
    ...
    $emailreplies = run("users:flags:get",array("emailnotifications",$page_owner));
    if ($emailreplies) {
        $body .= templates_draw(array(
            'context' => 'databox',
            'name' => gettext("Receive notifications: "),
            'column1' => "<label><input type=\"radio\" name=\"receivenotifications\" value=\"yes\" checked=\"checked\" /> " . gettext("Yes") . "</label> <label><input type=\"radio\" name=\"receivenotifications\" value=\"no\" /> " . gettext("No") . "</label>"
        )
        );
    } else {
        $body .= templates_draw(array(
            'context' => 'databox',
            'name' => gettext("Receive notifications: "),
            'column1' => "<label><input type=\"radio\" name=\"receivenotifications\" value=\"yes\" /> " . gettext("Yes") . "</label> <label><input type=\"radio\" name=\"receivenotifications\" value=\"no\" checked=\"checked\" /> " . gettext("No") . "</label>"
        )
        );
    }
