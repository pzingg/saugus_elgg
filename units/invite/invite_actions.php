    ...
    // Add a new invite code
                     ...
                     if ($invitations == 0) {
                         if (!$account = get_record('users','email',$invite->email)) {
                             ...
                             if (!logged_on) {
                                 ...
                                 $greetingstext = $USER->name . " " . gettext("has invited you to join") ." $sitename, ". gettext("a learning landscape system.") ."";
                                 $subjectline = $USER->name . " " . gettext("has invited you to join") ." $sitename";
                             ...
                             $messages[] = sprintf(gettext("Your invitation was sent to %s at %s. It will be valid for seven days."),$strippedname,$invite->email);
                             email_to_user($invite,null,$subjectline,$emailmessage);
                         } else {
                             $messages[] = sprintf(gettext("User %s already has that email address. Invitation not sent."),$account->username);
                         }
                     } else {
     ...
     case "invite_join":
         ...
         if (isset($name) && isset($code)) {
             ...
             email_to_user($u,null,sprintf(gettext("Your %s account"),$sitename), 
                  sprintf(gettext("Thanks for joining %s!\n\nFor your records, your %s username and password are:\n\n\t"
     ...
     case "invite_password_request":        
         ...
         if (!empty($username)) {
             if ($user = get_record('users','username',trim($username),'user_type','person')) {
                 ...
                 email_to_user($user,null,sprintf(gettext("Verify your %s account password request"),$sitename), 
                               sprintf(gettext("A request has been received to generate your account at %s a new password.\n\n"
