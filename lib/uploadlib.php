...
class upload_manager {
    ...
    */
            ...
            if (empty($this->inputname) || $name == $this->inputname) { // if we have input name, only process if it matches.
                ...
                if (!$this->status) {
                    if (!$this->config->recoverifmultiple && count($this->files) > 1) {
                        ...
                        $msg = sprintf(gettext('Your file upload has failed because there was a problem with one of the files, %s.<br /> Here is a log of the problems:<br />%s<br />Not recovering.'), $a->name, $a->problem);
                    ...
                    if ($newname != $this->files[$name]['name']) {
                        ...
                        $a->newname = $newname;
                        $this->files[$name]['uploadlog'] .= sprintf(gettext('File was renamed from %s to %s because of invalid characters.'), $a->oldname, $a->newname);
                    }
    ...
    function save_files($destination) {
        ...
        if (empty($savedsomething)) {
            ...
            if ((empty($this->config->allownull) && !empty($this->inputname)) || (empty($this->inputname) && empty($this->config->allownullmultiple))) {
                notify(gettext('No file was found - are you sure you selected one to upload?'));
            }
            return false;
        }
        return $this->status;
    ...
    function delete_other_files($destination, $exceptions=null) {
        ...
        if ($deletedsomething) {
            $msg = gettext('The old file(s) in your upload area have been deleted');
            if (!$this->config->silent) {
                notify($msg);
            }
            else {
    ...
    function get_file_upload_error(&$file) {
        ...
        case 0: // UPLOAD_ERR_OK
            if ($file['size'] > 0) {
                $errmessage = sprintf(gettext('An unknown problem occurred while uploading the file \'%s\' (perhaps it was too large?)'), $file['name']);
            } else {
                $errmessage = gettext('No file was found - are you sure you selected one to upload?'); /// probably a dud file name
            }
            break;
            
        case 1: // UPLOAD_ERR_INI_SIZE
            $errmessage = gettext('Uploaded file exceeded the maximum size limit set by the server');
            break;
            
        case 2: // UPLOAD_ERR_FORM_SIZE
            $errmessage = gettext('Uploaded file exceeded the maximum size limit set by the form');
            break;
            
        case 3: // UPLOAD_ERR_PARTIAL
            $errmessage = gettext('File was only partially uploaded');
            break;
            
        case 4: // UPLOAD_ERR_NO_FILE
            $errmessage = gettext('No file was found - are you sure you selected one to upload?');
            break;
    ...
    function print_upload_log($return=false,$skipemptyifmultiple=false) {
        foreach (array_keys($this->files) as $i => $key) {
            ...
            $str .= '<strong>'. sprintf(gettext('Upload log for file %u'), $i+1) .' '
...
function upload_print_form_fragment($numfiles=1, $names=null, $descriptions=null, $uselabels=false, $labelnames=null, $maxbytes=0, $return=false) {
    ...
    for ($i = 0; $i < $numfiles; $i++) {
        ...
        if ($uselabels) {
            ...
            $str .= gettext('Title:').' <input type="text" size="50" name="'. $lname .'" alt="'. $lname
                .'" /><br /><br />'."\n";
        }
    }
    if ($return) {
...
function clam_replace_infected_file($file) {
    $newcontents = gettext('This file that has been uploaded was found to contain a virus and has been moved or delted and the user notified.');
    if (!$f = fopen($file, 'w')) {
        return false;
    }
...
function clam_scan_file(&$file) {
    ...
    if (!$CFG->pathtoclam || !file_exists($CFG->pathtoclam) || !is_executable($CFG->pathtoclam)) {
        $newreturn = 1;
        $notice = sprintf(gettext('Elgg is configured to run clam on file upload, but the path supplied to Clam AV, %s,  is invalid.'), $CFG->pathtoclam);
    ...
    case 1:  // bad wicked evil, we have a virus.
        $info->user = $USER->name;
        $notice = sprintf(gettext('Attention administrator! Clam AV has found a virus in a file uploaded by %s. Here is the output of clamscan:'), $info->user);
        $notice .= "\n\n". implode("\n", $output);
        $notice .= "\n\n". clam_handle_infected_file($fullpath); 
        clam_mail_admins($notice);
        if ($appendlog) {
    ...
    default: 
        ...
        $notice .= sprintf(gettext('Clam AV has failed to run.  The return error message was %s. Here is the output from Clam:'), get_clam_error_code($return));
        $notice .= "\n\n". implode("\n", $output);
        $newreturn = true;
        if ($CFG->clamfailureonupload == 'actlikevirus') {
            $notice .= "\n". clam_handle_infected_file($fullpath);
...
function clam_mail_admins($notice) {
    ...
    $subject = sprintf(gettext('%s :: Clam AV notification'), $CFG->sitename);
    $user = new StdClass;
    $user->email = $CFG->sysadminemail;
    $user->name = $CFG->sitename.' '.gettext('Administrator');
...
function get_clam_error_code($returncode) {
    ...
    return gettext('There was an unknown error with clam.');
