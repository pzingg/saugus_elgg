...
function report_session_error() {
    ...
    setcookie('ElggSession'.$CFG->sessioncookie, '', time() - 3600, '/');
    setcookie('ElggSessionTest'.$CFG->sessioncookie, '', time() - 3600, '/');
    ...
    redirect($FULLME, gettext('A server error that affects your login session was detected. Please login again or restart your browser.'), 5);
}
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml='', $attachment='', $attachname='', $usetrueaddress=true, $repyto='', $replytoname='') {
    ...
    */
    ...
    if (is_string($from)) { // So we can pass whatever we want if there is need
        ...
        $mail->FromName = $CFG->sitename.' '.gettext('Administrator');
        ...
        $mail->FromName = $from->name;
        if (empty($replyto)) {
            $mail->AddReplyTo($CFG->noreplyaddress,gettext('Do not reply'));
        }
    }
    ...
    if ($attachment && $attachname) {
        if (ereg( "\\.\\." ,$attachment )) {    // Security check for ".." in dir path
            $mail->AddAddress($CFG->sysadminemail,$CFG->sitename.' '.gettext('Administrator'));
...
function close_window_button($name='closewindow') {
    ...
    echo "document.write('<input type=\"button\" onclick=\"self.close();\" value=\"".gettext("Close this window")."\" />');\n";
    echo "document.write('<\/form>');\n";
    echo '-->' . "\n";
    echo '</script>' . "\n";
    echo '<noscript>' . "\n";
...
function validate_email($address) {
    ...
    if (ereg('^[-!#$%&\'*+\\./0-9=?A-Z^_`a-z{|}~]+'.
                  ...
                  $address)) {
                      ...
                      if ($CFG->emailfilter != "") {
                          $domain = substr($address,strpos($address,"@"));
                          if (substr_count($domain,$CFG->emailfilter) == 0) {
                              return false;
                          }
                      }
...
function format_text_menu() {
    ...
    return array (FORMAT_MOODLE => gettext('Elgg auto-format'),
                  FORMAT_HTML   => gettext('HTML format'),
                  FORMAT_PLAIN  => gettext('Plain text format'),
                  FORMAT_MARKDOWN  => gettext('Markdown format'));
}
...
function print_continue($link) {
    ...
    print_single_button($link, NULL, gettext('Continue'), 'post', $CFG->framename);
    echo '</div>'."\n";
}
...
/**
 *Returns an A link tag using gettext()
 *
 * @param $url
 * @param $str for gettext()
 * @param $extrattr Extra attribs for the A tag
 * @return string
 */
function a_hrefg($url, $str, $extrattr='') {
    $str = htmlspecialchars(gettext($str), ENT_COMPAT, 'utf-8');
    return "<a href=\"$url\" $extrattr >$str</a>"; 
}
...
function notice_yesno ($message, $linkyes, $linkno) {
    ...
    print_single_button($linkyes, NULL, gettext('Yes'), 'post', $CFG->framename);
    echo '</td><td>';
    print_single_button($linkno, NULL, gettext('No'), 'post', $CFG->framename);
    echo '</td></tr></table>';
    print_simple_box_end();
}
...
function redirect($url, $message='', $delay='0') {
    ...
    if (empty($message)) {
        ...
        echo '<div style="text-align: center;">';
        echo '<p>'. $message .'</p>';
        echo '<p>( <a href="'. $encodedurl .'">'. gettext('Continue') .'</a> )</p>';
        echo '</div>';
...
function require_login() {
    ...
    if (!empty($CFG->tracksessionip)) {
        if ($USER->sessionIP != md5(getremoteaddr())) {
            error(gettext('Sorry, but your IP number seems to have changed from when you first logged in.  This security feature prevents crackers stealing your identity while logged in to this site.  Normal users should not be seeing this message - please ask the site administrator for help.'));
        }
    }
...
function remember_login($id) {
    ...
    global $messages;
    $messages[] = gettext("The system will remember you and automatically log you in next time.");
