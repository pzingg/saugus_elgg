...
if (empty($USER->signingup)) {
    ...
    if ($mode == 'join') {
        ...
        if (empty($u->name)) {
            $messages[] = gettext("Error! You must enter your fullname");
        }
        ...
        if (!authenticate_account($u->username, md5($u->password1))) {
            $messages[] = gettext('Error! Your username and password does not match our record.');
        }
        $USER->alias = $alias;
        $ident = $USER->ident;
    } elseif (!empty($mode)) {
        $messages[] = gettext('Fatal Error! Unknown action requested.');
    }
    ...
    if (empty($messages)) {
        ...
        if ($mode == 'join') { // we don't need to do these if the user has already had an account.
            $_SESSION['messages'][] = gettext('Your account creation was successful!');
            // authenticate them.
